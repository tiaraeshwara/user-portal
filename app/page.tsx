...
export default function Home() {
  ...
  const [foundUser, setFoundUser] = useState<{ id: string; name: string; email: string; city: string; age: number } | null>({
    id: "NODE-882-X",
    name: "Alex Rivera",
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-[1600px] mx-auto relative z-10">
        ...
        <header className="mb-12 flex flex-col md:flex-row justify-between items-center border-b border-white/5 pb-8">
          <div className="text-center md:text-left">
            ...
            <p className="text-cyan-200/40 text-[10px] font-bold uppercase tracking-[0.5em] mt-2">Central Intelligence Terminal</p>
          </div>
          <div className="h-12 w-12 glass-panel rounded-xl flex items-center justify-center text-2xl shadow-cyan-500/20 shadow-lg mt-4 md:mt-0">👨‍🚀</div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8">
          ...
          <motion.aside variants={itemVariants} className="lg:col-span-3 space-y-6">
            <div className="glass-panel rounded-[2rem] p-8 border-cyan-500/10 bg-cyan-500/[0.02]">
              <h3 className="text-xs font-black text-cyan-400 uppercase tracking-[0.2em] mb-4">Initialization</h3>
            ...
            <div className="glass-panel rounded-[2rem] p-8 opacity-50">
               <h4 className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-2">Network Load</h4>
            ...
            <div className="glass-panel rounded-[2.5rem] p-1 overflow-hidden h-full">
              <div className="p-8 bg-[#040916]/40 rounded-[2.3rem] h-full flex flex-col">
                <div className="flex justify-between items-center mb-8">
                  <h2 className="text-xl font-semibold text-white tracking-tight italic">Active Records</h2>
                  <div className="flex items-center gap-2 px-3 py-1 bg-cyan-500/5 border border-cyan-500/20 rounded-full">
                    ...
                    <span className="text-cyan-400 text-[9px] font-black uppercase tracking-tighter">Live</span>
                  </div>
                </div>
          ...
          <motion.aside variants={itemVariants} className="lg:col-span-3 space-y-6">
            <div className="glass-panel rounded-[2rem] p-8">
              <h3 className="text-xs font-black text-white uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                 <span className="w-2 h-2 bg-cyan-500 rounded-full" /> Global Search
              </h3>
              <UserSearch />
                  ...
                  {foundUser ? (
                    ...
                    >
                      <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/5">
                        <p className="text-[9px] font-black text-cyan-400 uppercase tracking-widest mb-1">Found Result</p>
                        <h4 className="text-lg font-bold text-white mb-0.5">{foundUser.name}</h4>
                        <p className="text-white/30 text-[10px] font-mono">{foundUser.id}</p>
                      </div>
                      ...
                      <button onClick={() => setFoundUser(null)} className="w-full text-[9px] text-white/20 hover:text-white/40 uppercase font-bold tracking-widest transition-colors">
                        Clear Search
                      </button>
                    </motion.div>
                  ) : (
      ...
      <AnimatePresence>
        {activeModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActiveModal(null)} className="absolute inset-0 bg-[#050b18]/90 backdrop-blur-xl" />
            <motion.div 
              ...
              <div className="p-10 md:p-14">
                <div className="flex justify-between items-center mb-10">
                   <h2 className="text-3xl font-black italic uppercase tracking-tighter">
                      {activeModal === 'add' ? "Init Node" : activeModal === 'update' ? "Modify Node" : "Purge Node"}
                   </h2>
                   <button onClick={() => setActiveModal(null)} className="text-white/30 hover:text-white transition-colors">✕</button>
                </div>
                ...
                <form className="space-y-6">
                  {activeModal !== 'delete' ? (
                    <>
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-1">
                          ...
                          <input type="text" className="form-input" placeholder="Name" defaultValue={activeModal === 'update' ? foundUser?.name.split(' ')[0] : ''} />
                        </div>
                        <div className="space-y-1">
                          <label className="form-label">Last Name</label>
                          <input type="text" className="form-input" placeholder="Surname" defaultValue={activeModal === 'update' ? foundUser?.name.split(' ')[1] : ''} />
                        </div>
                      </div>
                      <div className="grid grid-cols-3 gap-6">
                        <div className="space-y-1">
                          <label className="form-label">Age</label>
                          <input type="number" className="form-input" placeholder="00" defaultValue={activeModal === 'update' ? foundUser?.age : ''} />
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                          ...
                          <input type="text" className="form-input" placeholder="Location" defaultValue={activeModal === 'update' ? foundUser?.city : ''} />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-1">
                          <label className="form-label">Phone</label>
                          <input type="tel" className="form-input" placeholder="+00..." />
                        </div>
                        <div className="space-y-1">
                          <label className="form-label">Email</label>
                          <input type="email" className="form-input" placeholder="user@net.io" defaultValue={activeModal === 'update' ? foundUser?.email : ''} />
                        </div>
                      </div>
                    </>
                  ) : (
                    <div className="py-8 text-center space-y-6">
                       ...
                       <p className="text-white/40 italic">Confirm permanent wipe for <span className="text-white font-bold">{foundUser?.id}</span>?</p>
                       <input type="text" placeholder="Type CONFIRM to authorize" className="form-input text-center" />
                    </div>
                  )}

                  <button className={`w-full py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] transition-all shadow-xl mt-8 ${
                    activeModal === 'delete' ? 'bg-red-600 hover:bg-red-500 shadow-red-500/20' : 'bg-cyan-400 hover:bg-cyan-300 text-black shadow-cyan-500/20'
                  }`}>
                    Process Transaction
                  </button>
                </form>
